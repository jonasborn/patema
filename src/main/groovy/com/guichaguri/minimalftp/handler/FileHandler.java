 ...
 * Added file upload notification
 * @author Jonas Born
 */
...
public class FileHandler {
    ...
    private void stor(String path) throws IOException {
        ...
        receiveStream(file, fs.writeFile(file, start)); //Changed - Born
        start = 0;
    }
    ...
    private void stou(String[] args) throws IOException {
        ...
        receiveStream(file, fs.writeFile(file, 0));
    }
    ...
    private void appe(String path) throws IOException {
        ...
        con.sendResponse(150, "Receiving a file stream for " + path);
        receiveStream(file, fs.writeFile(file, fs.exists(file) ? fs.getSize(file) : 0));
    }
     ...
     */
    private void receiveStream(Object file, OutputStream out) {
        new Thread(() -> {
            try {
                con.receiveData(out);
                fs.finishedFile(file);
